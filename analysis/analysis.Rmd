...
g + geom_text(data = stg,
              aes(x = x, y = 0.85, label = round(y,1)),
              color = "blue", size=3,
              vjust = 1)  +
  # facet_grid(. ~ userDriven)+
   labs(y="Score", x = 'Feedback Question')
...
df_acc$custom_group <- ifelse(df_acc$taskDifficulty=='easy', "easy", "medium_hard")
df_time$custom_group <- ifelse(df_acc$taskDifficulty=='easy', "easy", "medium_hard")
...
df_medium_hard %>% 
  ...
  # ylim(0,100) +
   # ylim(0,100) +
  coord_flip() +
    facet_grid(taskType ~ custom_group)+
    theme(strip.text.y = element_text(angle=0))


filename <- paste("results.pdf")
 ggsave(filename, width = 12, height = 4, units = "in")
