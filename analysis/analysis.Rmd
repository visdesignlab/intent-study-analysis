...
filteredData <- dfp %>% 
  ...
  ggplot( aes(x=measure, y=value) ) +
    geom_jitter( shape=1 , position = position_jitter(w = 0.1, h = 0)) +
...
g<- df_acc %>% 
  ggplot( aes(x=group, y=value) ) +
    geom_jitter( shape=1 , aes(color=taskDifficulty, fill=taskDifficulty), position = position_jitter(w = 0.1, h = 0)) +
    stat_summary(fun.data = "mean_cl_boot", colour = "red", size = 0.5, alpha=0.5) +
  # ylim(0,100) +
   # xlim(-.25,.25) +
  coord_flip() +
...
# testData <- ggplot_build(g)$data
stt <- ggplot_build(g)$data[[2]]
print (stt)

# //add userDriven column

stt$userDriven <- ifelse(stt$PANEL==1, "manual", "supported")
g + geom_text(data = stt,
              aes(x = x, y = 0.1, label = round(y,3)),
              color = "blue", size=3,
              vjust = 1)  +
  facet_grid(. ~ userDriven)+
   labs(y="Accuracy", x = 'Task Type')


g<- df_acc %>% 
  ggplot( aes(x=taskDifficulty, y=value) ) +
    geom_jitter( shape=1 , aes(color=taskDifficulty, fill=taskDifficulty), position = position_jitter(w = 0.1, h = 0)) +
    stat_summary(fun.data = "mean_cl_boot", colour = "red", size = 0.5, alpha=0.5) +
  # ylim(0,100) +
   # xlim(-.25,.25) +
