...
filteredData <- df %>% 
  filter(measure=="difficulty" | measure == "confidence" | measure=="accuracy" | measure == "secondsOnTask")
...
ggplot(dfp_filtered, aes(x=value)) +
  geom_histogram() +
   # coord_flip() +
      stat_summary(fun.data = "mean_cl_boot", aes(x = 0.1, y = value, xintercept = stat(y)),geom = "vline",  colour = "red", size = 0.5, alpha=0.5) +
...
ggplot( aes(x=value)) +
 ...
 ggsave('demo_employment', width = 6, height = 4, units = "in")
