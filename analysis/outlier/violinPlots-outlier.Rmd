...
df <- read.csv("../../results/outlier/CSV/TidyR.csv")%>% 
...
df_time<- filter( df, measure == "secondsOnTask")
df_time$value = as.numeric( as.character(df_time$value ))


#filter out measurement that don't make the cut as far as timespent;
# dfv <- filter( df, measure != 'secondsOnTask' | as.numeric( as.character(value))<10)

dfp <- read.csv("../../results/outlier/CSV/participantInfoTidyR.csv")

# wilcox <- read.csv("../results/pilot/CSV/wilcox.csv")
# ci_values <- read.csv("../results/pilot/CSV/stats.csv")
...
for (task in seq(12,12,1)){
  citext(paste("S-task", sprintf("%.2d",task),sep=""),"secondsOnTask")
}
...
for (task in seq(1,16,1)){
  estext(paste("S-task", sprintf("%.2d",task),sep=""),"secondsOnTask")
}
```
...
for (task in seq(1,16,1)){
  print (wtest(paste("S-task", sprintf("%.2d",task),sep=""),"secondsOnTask"))
}
...
tt = as.character(sdf[1,]$taskId)
prompt = as.character(sdf[1,]$taskPrompt)
number = as.character(sdf[1,]$taskId)
...
plotData <-df %>% 
  filter( measure == "accuracy" | measure == "secondsOnTask"  )  %>% 
   ...
   mutate( label = labels.measure[as.character(measure)] ) %>% 
      mutate( visLabel = labels.visType[as.character(userDriven)] ) %>% 

  mutate( label = factor(label, levels=c("Accuracy","Time (minutes)")));
...
print(nrow(plotData))
  ...
  ggplot(data=plotData, aes(x=userDriven, y=value, fill=taskType, color=taskType)) +
...
filename <- paste("violins/",gsub(" ", "", number, fixed = TRUE),".pdf", sep="")
 ggsave(filename, width = 14, height = 1.5, units = "in")
}

 # vplot('0')

for (task in seq(0,50,1)){
  sprintf("%.1d",task)
 # vplot(paste(sprintf("%.1d",task),sep=""))
}
