...
df$custom_group <- ifelse(df_acc$taskDifficulty=='easy', "easy", "medium_hard")
...
wtest(df_medium_hard,"skyline")


wtest(df_easy,"cluster")
wtest(df_easy,"linear regression")
wtest(df_easy,"outlier")
wtest(df_easy,"quadratic regression")
wtest(df_easy,"skyline")
```
...
axisRange <- function(x) { 
  if (max(as.numeric(x))<2) c(0,1)
   else  if (max(as.numeric(x))<10) c(1,5)
  else (c(0,100))
}
# 
...
g<- df_easy %>% 
  ggplot( aes(x=userLabel, y=value, fill = userLabel, color = userLabel) ) +
    facet_grid(taskType_f ~ measure, scales="free")+
...
stt$taskType <- ifelse((stt$PANEL <5), "Cluster", ifelse((stt$PANEL <9), "Linear Regression", ifelse((stt$PANEL <12), "Quadratic Regression", ifelse((stt$PANEL <17), "Multivariate Optimization", "Outlier"))))
...
stt$measure <- ifelse((stt$PANEL %% 4 == 0), "Accuracy", ifelse((stt$PANEL %% 4 == 1), "Time (seconds)", ifelse((stt$PANEL %% 4 == 2), "Confidence", "Difficulty")))
...
g + geom_text(data = stt,
              aes(x =userLabel, y = 0.5,label = paste(round(y,3) , '[' , round(ymin,3) ,  ',' , round(ymax,3) , ']',sep=" ")),
             color = "#5c5c5c",size=2,
              vjust = 1)  +
   facet_grid(taskType_f ~ measure,scales = "free")+
    scale_y_continuous(breaks = gridLines, limits = axisRange) #,labels = gridLabels) 
   ...
   # labs(y="Accuracy", x = '')
# 
# filename <- paste("results_violin_color_easy.pdf")
#  ggsave(filename, width = 6, height = 5, units = "in")
